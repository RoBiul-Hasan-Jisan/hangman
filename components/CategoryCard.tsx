...
export default function CategoryCard({
  ...
  return (
    <div
      ref={cardRef}
      onClick={onClick}
      className={`glass cursor-pointer group relative overflow-hidden border-2 ${bgGradients[name.toLowerCase()] || bgGradients.animals} h-full p-6 md:p-8`}
    >
      ...
      <div ref={contentRef} className="relative z-10 flex flex-col h-full">
        ...
        {/* Category Name */}
        <h3 className="text-2xl md:text-3xl font-bold text-white mb-4 capitalize group-hover:gradient-text">
          {info.name}
        </h3>
        ...
        <div className="mt-4 pt-4 border-t border-slate-500/50">
          <button
            ...
            className="w-full py-2 px-4 bg-gradient-to-r from-blue-500 to-purple-500 text-white font-bold rounded-lg opacity-0 group-hover:opacity-100"
          >
            Play Now
          </button>
        </div>
